#!/usr/bin/env python3
"""
Code Admit Gate — Snapshot & Restore
...
def build_manifest(repo: Path, forbidden: List[str]) -> Dict[str, Any]:
    ...
    return {
        "schema": "code_admit_gate.snapshot_manifest.v1",
        "created_at": now_iso(),
        "repo": str(repo),
...
def write_archive(repo: Path, manifest: Dict[str, Any], archive_path: Path) -> None:
    ...
    with tarfile.open(archive_path, "w:gz") as tar:
        ...
        info = tarfile.TarInfo(name="_code-admit-gate-manifest.json")
        info.size = len(data)
        info.mtime = 0
...
def is_safe_member(repo: Path, member: tarfile.TarInfo) -> bool:
    if member.name == "_code-admit-gate-manifest.json":
        return False
...
def do_restore(archive_path: Path, repo: Path, verify_only: bool) -> int:
    ...
    with tarfile.open(archive_path, "r:gz") as tar:
        names = tar.getnames()
        manifest = None
        if "_code-admit-gate-manifest.json" in names:
            manifest_member = tar.extractfile("_code-admit-gate-manifest.json")
            if manifest_member:
        ...
        unsafe_count = len(tar.getmembers()) - len(members) - (1 if "_code-admit-gate-manifest.json" in names else 0)
        if unsafe_count:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Code Admit Gate — snapshot and restore")
    parser.add_argument("--repo", default=".", help="Repository root")
    parser.add_argument("--out", default=".code-admit-gate/snapshot", help="Output path stem for snapshot mode")
